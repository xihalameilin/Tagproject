   ...
   created(){
    ...
    methods: {
      ...
      mark(index){
        ...
        var messageid=this.unreadMesList[index].id
        this.unreadMesList.splice(index,1)
        this.$http.get("api/worker/mark/"+messageid)
      },

        getMessage(){
          var self=this
          var userID = window.localStorage.getItem("userID")
          this.$http.get('api/worker/getMessageUnRead/'+userID)
            .then(function (response) {
              ...
              self.currentMesList=self.unreadMesList=array
            })
        },
        formatDate (time) {
