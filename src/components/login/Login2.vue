      ...
      <Form v-show="tagdisplay.flag2" ref="registerformCustom" :model="registerformCustom" :rules="ruleCustom" :label-width="80">
        ...
        <FormItem label="身份：" >
          <RadioGroup v-model="registerformCustom.registerIdentity">
            <Radio label="普通工人"></Radio>
            <Radio label="任务发起者"></Radio>
          </RadioGroup>
        </FormItem>
        <FormItem>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$http.get('api/LoginController/Login/'+this.formCustom.userID+"/"+this.formCustom.password,{
          ...
          .then(function (response) {
            ...
            if(identity=='管理员') {
              ...
              window.localStorage.setItem("identity","工人")
              self.$router.push({
                name: 'worker',
              })
            }
            else if(identity=='发起者'){
              window.localStorage.setItem("identity","发起者")
              self.$router.push({
                name: 'requester',
              })
            }
      ...
      //注册
      register(){
        if(this.userIDExitable==true){
          this.$Message.warning("用户名已经存在不可以注册")
        }else if(this.EmailExitable==true){
          this.$Message.warning("邮箱已经存在不可以注册")
        }else {
          var identity = 1
          if (this.registerformCustom.registerIdentity == '普通工人')
            identity = 0
          var self = this
          this.$http.post('api/LoginController/Register/'
            + this.registerformCustom.registeruserID + '/'
            + this.registerformCustom.registerPassword + '/' + identity + "/" + self.registerformCustom.registerCode + "/" + self.registerformCustom.registerEmail, {})
            .then(function (response) {
              var m = self
              var flag = response.data
              if (flag == '2') {
                m.$Message.error("验证码错误")
              } else {
                m.changetag(0)
              }
            })
